...
get_node_url(){
	...
	download_Url1='https://raw.githubusercontent.com/lhpmain/btpanel_crack/master/bt_6.9';
	echo "Download node: $download_Url";
	echo "Download node: $download_Url1";
	echo '---------------------------------------------';
}
Remove_Package(){
	local PackageNmae=$1
...
Install_Bt(){
    ...
    # https://raw.githubusercontent.com/lhpmain/btpanel_crack/master/bt_6.9';
	# wget -O panel.zip ${download_Url}/install/src/panel6.zip -T 10
	wget -O panel.zip ${download_Url1}/panel6.9.zip -T 10
	wget -O /etc/init.d/bt ${download_Url1}/install/src/bt6.init -T 10
	chattr -i /www/server/panel/install/public.sh
	chattr -i /www/server/panel/install/check.sh
	wget -O /www/server/panel/install/public.sh ${download_Url1}/public.sh -T 10
    ...
    chattr +i /www/server/panel/install/public.sh
	wget -O /www/server/panel/install/check.sh ${download_Url1}/check.sh -T 10
	chattr +i /www/server/panel/install/check.sh
	rm -f panel.zip
...
Install_Pillow()
{
	...
	if [ "$isSetup" = "" ];then
		...
		wget -O Pillow-3.2.0.zip $download_Url1/Pillow-3.2.0.zip -T 10
...
Install_psutil()
{
	...
	if [ "$isSetup" = "" ];then
		wget -O psutil-5.2.2.tar.gz $download_Url1/psutil-5.2.2.tar.gz -T 10
...
Install_chardet()
{
	...
	if [ "${isSetup}" = "" ];then
		wget -O chardet-2.3.0.tar.gz $download_Url1/chardet-2.3.0.tar.gz -T 10
